...
type Ipc struct {
	...
	log            *zerolog.Logger
	cfg            INatsConfig
	listeners      map[string][]dispatcher.IDispatcherIpcController
	listenersQueue map[string][]dispatcher.IDispatcherIpcController
}
...
func IpcCreate(cfg INatsConfig, log *zerolog.Logger) *Ipc {
	...
	ipc := &Ipc{
		...
		log:       log,
		cfg:       cfg,
		listeners: make(map[string][]dispatcher.IDispatcherIpcController),
	}
	ipc.Connect()
	return ipc
}
...
func (i *Ipc) Subscribe(subj string, listener dispatcher.IDispatcherIpcController) (err fault.IError) {
...
func (i *Ipc) QueueSubscribe(subj, queue string, listener dispatcher.IDispatcherIpcController) (err fault.IError) {
...
func (i *Ipc) Request(subj string, data []byte, timeout time.Duration) (resp dispatcher.IIpcMessage, err fault.IError) {
	// if i.conn.IsClosed() || !i.conn.IsConnected() {
	// 	i.Connect()
	// }
